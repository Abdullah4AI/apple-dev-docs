package commands

import (
	"github.com/Abdullah4AI/apple-developer-toolkit/swiftship/internal/nwtool"
	"github.com/Abdullah4AI/apple-developer-toolkit/swiftship/internal/revenuecatserver"
	"github.com/Abdullah4AI/apple-developer-toolkit/swiftship/internal/supabaseserver"
	"github.com/Abdullah4AI/apple-developer-toolkit/swiftship/internal/xcodegenserver"
	"github.com/spf13/cobra"
)

var mcpCmd = &cobra.Command{
	Use:    "mcp",
	Short:  "Run MCP servers (used internally by supported agent runtimes)",
	Hidden: true,
}

var mcpXcodegenCmd = &cobra.Command{
	Use:   "xcodegen",
	Short: "Run the XcodeGen MCP server",
	Long:  "Starts the XcodeGen MCP server over stdio. Used by supported agent runtimes to manage Xcode project configuration via typed tool calls.",
	RunE: func(cmd *cobra.Command, args []string) error {
		return xcodegenserver.Run(cmd.Context())
	},
}

var mcpSupabaseCmd = &cobra.Command{
	Use:   "supabase",
	Short: "Run the Supabase MCP server",
	Long:  "Starts the Supabase MCP server over stdio. Used by supported agent runtimes to manage Supabase backend via the Management API.",
	RunE: func(cmd *cobra.Command, args []string) error {
		return supabaseserver.Run(cmd.Context())
	},
}

var mcpRevenuecatCmd = &cobra.Command{
	Use:   "revenuecat",
	Short: "Run the RevenueCat MCP server",
	Long:  "Starts the RevenueCat MCP server over stdio. Used by supported agent runtimes to manage RevenueCat products, entitlements, and offerings via the REST API v2.",
	RunE: func(cmd *cobra.Command, args []string) error {
		return revenuecatserver.Run(cmd.Context())
	},
}

var mcpToolsCmd = &cobra.Command{
	Use:   "tools",
	Short: "Run the nanowave tools MCP server",
	RunE: func(cmd *cobra.Command, args []string) error {
		return nwtool.RunMCPServer(cmd.Context())
	},
}

func init() {
	mcpCmd.AddCommand(mcpXcodegenCmd)
	mcpCmd.AddCommand(mcpSupabaseCmd)
	mcpCmd.AddCommand(mcpRevenuecatCmd)
	mcpCmd.AddCommand(mcpToolsCmd)
}
