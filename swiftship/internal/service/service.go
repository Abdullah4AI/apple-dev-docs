...
func NewService(cfg *config.Config, opts ...ServiceOpts) (*Service, error) {
	...
	return &Service{
		config:       cfg,
		claude:       claudeClient,
		projectStore: storage.NewProjectStore(cfg.SwiftShipDir),
		historyStore: storage.NewHistoryStore(cfg.SwiftShipDir),
		usageStore:   storage.NewUsageStore(cfg.SwiftShipDir),
		model:        model,
	}, nil
}
...
func (s *Service) UpdateConfig(cfg *config.Config) {
	s.config = cfg
	s.projectStore = storage.NewProjectStore(cfg.SwiftShipDir)
	s.historyStore = storage.NewHistoryStore(cfg.SwiftShipDir)
	s.usageStore = storage.NewUsageStore(cfg.SwiftShipDir)
}
...
func (s *Service) build(ctx context.Context, prompt string, images []string) error {
	terminal.Header("SwiftShip Build")
	...
	s.projectStore = storage.NewProjectStore(s.config.SwiftShipDir)
	s.historyStore = storage.NewHistoryStore(s.config.SwiftShipDir)
	s.usageStore = storage.NewUsageStore(s.config.SwiftShipDir)

	// Record usage
	s.usageStore.RecordUsage(result.TotalCostUSD, result.InputTokens, result.OutputTokens, result.CacheRead, result.CacheCreated)

	// Save state
	if err := s.config.EnsureSwiftShipDir(); err == nil {
...
func (s *Service) edit(ctx context.Context, prompt string, images []string) error {
	...
	terminal.Header("SwiftShip Edit")
...
func (s *Service) Fix(ctx context.Context) error {
	project, err := s.projectStore.Load()
	if err != nil || project == nil {
		return fmt.Errorf("no active project found. Run `swiftship` first")
	}

	terminal.Header("SwiftShip Fix")
	terminal.Detail("Project", projectName(project))
...
func (s *Service) Run(ctx context.Context) error {
	...
	if err == nil {
		...
		if retryErr != nil {
			...
			terminal.Info("Run `swiftship fix` to try again")
	...
	if bundleID == "" {
		bundleID = fmt.Sprintf("com.swiftship.%s", strings.ToLower(scheme))
	}
	...
	if watchDuration > 0 {
		...
		terminal.Detail("Tip", "Set SWIFTSHIP_RUN_LOG_WATCH_SECONDS=0 to disable log watching")
		...
		terminal.Detail("Tip", "Set SWIFTSHIP_RUN_LOG_WATCH_SECONDS=0 to disable or a positive value for timed log watching")
		if streamErr := streamSimulatorLogs(ctx, scheme, bundleID, watchDuration); streamErr != nil {
			terminal.Warning(fmt.Sprintf("Log streaming unavailable: %v", streamErr))
		}
	}
...
func runLogWatchDuration() time.Duration {
	raw := strings.TrimSpace(os.Getenv("SWIFTSHIP_RUN_LOG_WATCH_SECONDS"))
	if raw == "" {
