...
var setupCmd = &cobra.Command{
	...
	Long:  "Check and install all prerequisites needed to use SwiftShip.",
...
func runSetup() error {
	terminal.Header("SwiftShip Setup")
	fmt.Println()
	...
	if config.CheckXcodeCLT() {
		...
		if askConfirm(reader, "    Install Xcode Command Line Tools?") {
			...
			if err := installCmd.Start(); err != nil {
				...
				terminal.Info("installation dialog opened. Complete the install and re-run `swiftship setup`.")
			}
		} else {
			terminal.Detail("Install manually", "xcode-select --install")
		}
	...
	if allGood {
		...
		terminal.Warning("Some prerequisites are missing. Install them and run `swiftship setup` again.")
	}
