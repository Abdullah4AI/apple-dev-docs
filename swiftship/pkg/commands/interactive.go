...
func newImageCache() (*imageCache, error) {
	dir, err := os.MkdirTemp("", "swiftship-images-*")
	if err != nil {
		return nil, err
	}
...
func runInteractive(cmd *cobra.Command) error {
	...
	go func() {
		updateCh <- update.Check("Abdullah4AI", "swiftship", Version)
	}()
	...
	case res := <-updateCh:
		if res.NeedsUpdate() {
			terminal.Warning(fmt.Sprintf("Update available: v%s → v%s  —  run `brew upgrade swiftship`", res.Current, res.Latest))
			fmt.Println()
		}
	...
	if err != nil {
		...
		terminal.Info("Run `swiftship setup` to install all prerequisites.")
		return err
	}
