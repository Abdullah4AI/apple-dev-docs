...
{"authServiceKey":"[REDACTED]","serviceKey":"[REDACTED]","status":"failed"}`,
		...
		},
		{
			...
			input: "Google request failed for AIza0123456789abcde" + "fghijklmnopqrstuvwxy",
		...
		},
		{
			...
			input: "request failed with SG.abcdefghijklmnopqrstuv.ABCDEFGH" + "IJKLMNOPQRSTUVWXYZabcdefghijklmnopq",
...
kind: Secret
data:
  tls.key: b3BhcXVlLXBy` + `aXZhdGUta2V5
  ...
  data:
    config: public-config`,
		},
		{
			...
			input: `{"apiVersion":"v1","kind":"Secret","data":{"tls.key":"b3BhcXVlLXBy` + `aXZhdGUta2V5",".dockerconfigjson":"eyJhdXRocyI6eyJyZWdpc3RyeS5leGFtcGxlIjp7ImF1dGgiOiJvcGFxdWUifX19"},"status":"failed"}`,
...
status: failed`,
  ...
  set --value [REDACTED] --secret`,
		...
		},
		{
			...
			input: `curl --user alice:supe` + `rsensitive https://example.test`,
			want:  `curl --user [REDACTED] https://example.test`,
		},
		{
			name:  "curl user password in separate quoted shell fragment",
			input: `curl --user ali` + `ce:'` + `supersen` + `sitive password' https://example.test`,
			want:  `curl --user [REDACTED] https://example.test`,
		},
		{
			name:  "curl user with separately quoted username",
			input: `curl --user 'alice':sup` + `ersensitive https://example.test`,
			want:  `curl --user [REDACTED] https://example.test`,
		},
		{
		...
		},
		{
			...
			input: `openssl -provider default mac -digest SHA256 -macopt=hexkey:01234567` + `89abcdef HMAC`,
		...
		},
		{
			...
			input: `sudo -P openssl -provider default kdf -keylen 32 -kdfopt digest:SHA256 -kdfopt key:opaque-kdf-key -kdfopt=hexkey:01234567` + `89abcdef HKDF`,
		...
		},
		{
			...
			input: "openssl kdf \\\n-kdfopt pass:opaque-kdf-password -kdfopt hexpass:01234567" + "89abcdef PBKDF2",
		...
		},
		{
			...
			input: `ASC_STOREKIT_PRIVATE_KEY_B64=c3RvcmVraXQtcH` + `JpdmF0ZS1rZXk= ASC_ADS_PRIVATE_KEY_B64=YWRzLXByaX` + `ZhdGUta2V5`,
		...
		},
		{
			...
			input: `//registry.npmjs.org/:_auth=b3BhcXVlLX` + `NlY3JldA==`,
			want:  `//registry.npmjs.org/:_auth=[REDACTED]`,
		},
		{
			name:  "registry configuration auth value",
			input: `{"auths":{"registry.example":{"auth":"YWxpY2U` + `6b3BhcX` + `VlLXNlY3JldA=="}},"credsStore":"desktop"}`,
  ...
  - !!str "password": "[REDACTED]"`,
		...
		},
		{
			...
			input: "client-key-data: b3BhcXVlLXBy" + "aXZhdGUta2V5\nclient-certificate-data: public-certificate",
		...
		},
		{
			...
			input: "credential xoxb-123456789012-123456789" + "012-abcdefghijklmnopqrstuvwx",
			want:  "credential [REDACTED]",
		},
		{
		...
		},
		{
			...
			input: "credential xapp-1-123456789012-12345678" + "9012-abcdefghijklmnopqrstuvwx",
			want:  "credential [REDACTED]",
		},
		{
		...
		},
		{
			...
			input: "credential glpat-abcdefg" + "hijklmnopqrstuvwxyz",
			want:  "credential [REDACTED]",
		},
		{
...
func TestRedactSensitiveTextRedactsCombinedCurlCredentials(t *testing.T) {
	input := `curl --user alice:op` + `aque-user --cert client.p12:opaque-cert https://example.test`
...
func TestSnitchDryRunRedactsRegistryBase64AuthCredential(t *testing.T) {
	...
	const secret = "b3BhcXVlLX" + "NlY3JldA=="
	stdout, stderr, err := runSnitchCommand(
		t, "9.9.9",
		"--dry-run",
...
func TestSnitchDryRunRedactsKubernetesSecretData(t *testing.T) {
	...
	const privateKey = "b3BhcXVlLXBy" + "aXZhdGUta2V5"
...
func TestSnitchDryRunRedactsRegistryConfigurationAuthCredential(t *testing.T) {
	...
	const secret = "YWxpY2U" + "6b3BhcX" + "VlLXNlY3JldA=="
	stdout, stderr, err := runSnitchCommand(
		t, "9.9.9",
		"--dry-run",
...
func TestSnitchDryRunRedactsKubeconfigClientKeyData(t *testing.T) {
	...
	const secret = "b3BhcXVlLXBy" + "aXZhdGUta2V5"
	stdout, stderr, err := runSnitchCommand(
		t, "9.9.9",
		"--dry-run",
...
func TestSnitchDryRunRedactsEnvironmentDumpAndKnownServiceTokens(t *testing.T) {
	...
	secrets := []string{
		"opaque secret tail",
		"xoxb-1" + "2345678" + "9012-123456789" + "012-abcdefghijklmnopqrstuvwx",
		"npm_abcdefghijklmnopqrstuvwxyz0123456789",
		"glpat-abcdefg" + "hijklmnopqrstuvwxyz",
	}
	stdout, stderr, err := runSnitchCommand(
		t, "9.9.9",
		"--dry-run",
...
func TestSnitchDryRunRedactsGoHeaderMapsAndContinuedCurlCredentials(t *testing.T) {
	...
	stdout, stderr, err := runSnitchCommand(
		...
		"--repro", "curl --user alice:"+secrets[0]+"\\\n-tail https://example.test\n"+
			"curl --user ali"+"ce:'"+secrets[3]+" password' https://example.test/split",
