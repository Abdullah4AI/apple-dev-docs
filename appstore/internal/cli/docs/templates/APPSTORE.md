# appstore cli reference

Unofficial CLI for the App Store Connect API. AI-friendly command catalog and
workflow notes for the appstore cli. Use this alongside the appstore cli readme
(examples) and `appstore --help` (source of truth). Generate this file in any repo
with `appstore init` (or `appstore docs init`).
...
- `--app "APP_ID"` is often required (or set `APPSTORE_APP_ID`).
...
| Task | Command |
|------|---------|
| Check auth status | `appstore auth status` |
| Run auth doctor | `appstore doctor --output json` |
| Check account health | `appstore account status` |
| Generate APPSTORE.md | `appstore init` |
| List apps | `appstore apps` |
| List builds | `appstore builds list --app "APP_ID"` |
| List TestFlight apps | `appstore testflight apps list` |
| List beta groups | `appstore testflight beta-groups list --app "APP_ID"` |
| List internal beta groups | `appstore testflight beta-groups list --app "APP_ID" --internal` |
| Submit for review | `appstore submit create --app "APP_ID" --version "VERSION" --build "BUILD_ID" --confirm` |
| Weekly insights summary | `appstore insights weekly --app "APP_ID" --source analytics --week "YYYY-MM-DD"` |
| Download localizations | `appstore localizations download --version "VERSION_ID" --path "./localizations"` |
...
Use `appstore <command> --help` for subcommands and flags.
...
- `install-skills` - Install the appstore skill pack for App Store Connect workflows.
- `init` - Initialize appstore helper docs in the current repo.
- `docs` - Generate appstore cli reference docs for a repo.
...
- `APPSTORE_TIMEOUT`, `APPSTORE_TIMEOUT_SECONDS` - Request timeout
- `APPSTORE_UPLOAD_TIMEOUT`, `APPSTORE_UPLOAD_TIMEOUT_SECONDS` - Upload timeout
- `APPSTORE_DEBUG` - Debug output (`api` enables HTTP logs)
- `APPSTORE_SPINNER_DISABLED` - Disable interactive stderr spinner

## API References (Offline)

In the appstore cli repo, see:
